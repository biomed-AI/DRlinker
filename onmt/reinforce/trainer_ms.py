...
from onmt.reinforce.experience import Experience
...
class RL_ms_Trainer(object):
    ...
    def train(self, src_path, tgt_path, train_steps, batch_size, opt, logger=None):
        ...
        #因为batch_size为1 故反过来写 高效

        experience = Experience(self.vocab, info)

        for batch in data_iter:

            scoring_function_kwargs = {}
            # [torchtext.data.batch.Batch of size 1]
            # [.src]:('[torch.cuda.LongTensor of size 52x1 (GPU 0)]', '[torch.cuda.LongTensor of size 1 (GPU 0)]')
            # [.indices]:[torch.cuda.LongTensor of size 1(GPU 0)]
            # [.tgt]:[torch.cuda.LongTensor of size 69x1(GPU 0)]
            # [.src_map]:[torch.cuda.FloatTensor of size 52x1x16(GPU 0)]
            # [.alignment]:[torch.cuda.LongTensor of size 69x1(GPU 0)]

            # print (self.seq_to_smile(batch.src[0]))
            # print (self.seq_to_smile(batch.tgt))
            # print (self.seq_to_smile_tol_test(batch.src[0]))
            # print (self.seq_to_smile_tol_test(batch.tgt))
            # OCCOC.*c1cccc(C# C)c1
            # C#Cc1cccc(Nc2ncnc3cc(OCCOC)c(OCCOC)cc23)c1
            # *OCCOC.*c1cccc(C  # C)c1
            # < s > C# Cc1cccc(Nc2ncnc3cc(OCCOC)c(OCCOC)cc23)c1</s>
            ...
            # print('kwargs: ', scoring_function_kwargs['src'], '  ', scoring_function_kwargs['ref'])
            ...
            while cur_step <= train_steps:
                ...
                # start_token = self.vocab.stoi[inputters.BOS_WORD]
                # end_token = self.vocab.stoi[inputters.EOS_WORD]
                ...
                sequences = alive_seq.permute(1, 0)
                # 3.use prior to get the samples likelihood ,just like teacher force
                ...
                new_score = score
                new_experience = zip(smiles_sequences, alive_seq, score, prior_likelihood)
                experience.add_experience(new_experience)

                if len(experience) > 10:
                    exp_smiles, exp_seqs, exp_scores, exp_prior_likelihood = experience.sample(10)
                    exp_tgt = exp_seqs.permute(1, 0).unsqueeze(-1)[:-1]
                    exp_enc_states, exp_memory_bank, exp_src_lengths \
                        = self.agent.encoder(src, src_lengths)
                    exp_dec_states = self.agent.decoder.init_decoder_state(
                        src, exp_memory_bank, exp_enc_states, with_cache=True)
                    exp_dec_states.map_batch_fn(
                        lambda state, dim: tile(state, 10, dim=dim))
                    if type(exp_memory_bank) == tuple:
                        device = exp_memory_bank[0].device
                        exp_memory_bank = tuple(tile(m, 10, dim=1) for m in exp_memory_bank)
                    else:
                        exp_memory_bank = tile(exp_memory_bank, 10, dim=1)
                        device = exp_memory_bank.device
                    exp_memory_lengths = tile(exp_src_lengths, 10)

                    exp_outputs, exp_states, exp_attns = \
                        self.agent.decoder(exp_tgt, exp_memory_bank,
                                           exp_dec_states,
                                           memory_lengths=exp_memory_lengths)
                    exp_outputs = self.agent.generator(exp_outputs)
                    exp_gtruth = exp_seqs.permute(1, 0)[1:]
                    exp_likelihood = self.criterion_per_sample(exp_outputs, exp_gtruth)
                    exp_prior_likelihood = exp_prior_likelihood.cuda(device)
                    prior_likelihood = torch.cat((prior_likelihood, exp_prior_likelihood), dim=0)
                    agent_likelihood = torch.cat((agent_likelihood, exp_likelihood), dim=0)
                    new_score = np.concatenate((new_score, exp_scores))
                    smiles_sequences.extend(exp_smiles)

                # 对目前sample的结果加权重，直接计数，/count
                smiles_sequences_standard = self.to_standardSmiles(smiles_sequences)
                smiles_counts_dict = Counter(smiles_sequences_standard)
                weights = []
                for smi in smiles_sequences:
                    tmp_mol = Chem.MolFromSmiles(smi)
                    if tmp_mol is None:
                        weights.append(1.0 / smiles_counts_dict[smi])
                    else:
                        weights.append(1.0 / smiles_counts_dict[Chem.MolToSmiles(tmp_mol)])
                weights = self.toVariable(np.array(weights))


                # 7.linear the two loss
                ...
                if cur_step % opt.report_every == 0:
                    ...
                    info(
                        f'loss :::  {loss.data}, score: {new_score.mean()}, penalty score: {(new_score*weights.cpu().numpy()).mean()}, repeat sample: {len(weights) - int(weights.sum())}, lr: {self.optim.learning_rate, self.optim.original_lr} ')
                    info(
                ...
                #if cur_step % 50 == 0:
                #    experience.clear_memory()

                    # del smiles_sequences
                    # del smiles_sequences_standard
                    # gc.collect()
                del smiles_sequences,smiles_sequences_standard,\
                    enc_states,memory_bank,dec_states,alive_seq,\
                    dec_out, attn,agent_target,agent_scores,\
                    prior_enc_final, prior_memory_bank,prior_outputs, prior_states, prior_attns,\
                    weights
                gc.collect()
                torch.cuda.empty_cache()
                # if step % opt.save_checkpoint_steps == 0:
                #     makedirs(opt.save_model_path)
                #     save_checkpoint(opt.save_model, \
                #                     step, Agent, optim, fields, logger, agent_opt)
    ...
    def criterion_per_sample(self, inputs, targets):
        ...
        # 要求index维度要和input一致 也即(seq_length,batch_size, 1)=>(seq_length,batch_size, num_classes)
        target_expanded.scatter_(-1, targets, 1.0)
        non_pad_inputs =  (targets != self.vocab.stoi[PAD_WORD])
        #将target label 同样转换为 num_classes的one_hot形式
