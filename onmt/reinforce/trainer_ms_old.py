...
class RL_ms_Trainer(object):
        ...
        for batch in data_iter:
            ...
            while cur_step <= train_steps:
                ...
                for step in range(opt.max_length):
                    ...
                    # print(f'step{step}')
                    # print(f'dec_int,alive_seq:{decoder_input.size(), alive_seq.size()}')
                    # print(f'alive_seq[:,-1]: {alive_seq[:, -1]}')
                    # tmp = self.seq_to_smiles(alive_seq)
                    # print(f'alive_seq indexs 0 : {alive_seq[0,:]}')
                    # print(f'alive_seq indexs 1 : {alive_seq[1,:]}')
                    # print(f'alive_seq tol 0: {tmp[0]}')
                    # print(f'alive_seq tol 1: {tmp[1]}')
                    ...
                    # print(f'agent.generator log_probs size: {log_probs.size()}')
                    ...
                    # print(f'x x_new: {x.size(),x_new.size(),x,x_new}')
                    alive_seq = torch.cat(
                        [alive_seq, x_new.view(-1, 1)], -1)
                    EOS_sampled = (x_new.data == self.vocab.stoi[inputters.EOS_WORD]).data
                    finished = torch.ge(finished + EOS_sampled, 1)

                    # print(f'EOS_sampled{EOS_sampled.size(),EOS_sampled}; finished{finished.size(),finished}')
                ...
                # print('*'*20)
                # print(f'dec_outputs')
                # tmp = self.seq_to_smiles_tttol(alive_seq)
                # print(f'alive_seq indexs 0 : {alive_seq[0, :]}')
                # print(f'alive_seq indexs 1 : {alive_seq[1, :]}')
                # print(f'alive_seq tol 0: {tmp[0]}')
                # print(f'alive_seq tol 1: {tmp[1]}')
                # print(f'final alive_seq: {alive_seq.size()}')
                # print(dec_out.size(), log_probs.size(), agent_outputs.size())
                ...
                # print(f'xxxxxxxxxxxxx: {agent_scores.size(),agent_gtruth.size()}')
                # print(f'agent_likelihood: {agent_likelihood}')
                ...
                smiles_counts_dict = Counter(smiles_sequences)
                weights = []
                ...
                # print(f'prior_outputs: {prior_outputs.size()}')
                ...
                augmented_likelihood = prior_likelihood + opt.sigma * self.toVariable(score)
                loss = torch.pow((augmented_likelihood - agent_likelihood), 2)
                # print(loss, type(loss))
                # print(weights, type(weights))
                # print(score, type(score))
                # print(toVariable(score), type(toVariable(score)))
                # print(augmented_likelihood, type(augmented_likelihood))
                loss = loss.mul(self.toVariable(weights))
                ...
                if cur_step % opt.report_every == 0:
                    ...
                    info(
                        f'loss :::  {loss.data},  score: {(score * weights).mean()}, repeat sample: {len(weights) - int(weights.sum())}, lr: {self.optim.learning_rate, self.optim.original_lr} ')
                    info(
                    ...
                    for i in range(opt.n_best):
                        info(smiles_sequences[i])

                    # print(f'score: {score}')
                    # print(f'weights: {len(weights),weights.sum(),len(weights)-int(weights.sum())}')
                    # print(f'score* weights: {score*weights}')
                    # print(f'score: {(score*weights).mean()}')
    ...
    def criterion_per_sample(self, inputs, targets):
        ...
        target_expanded.scatter_(-1, targets.unsqueeze(-1).contiguous().data, 1.0)
        #将target label 同样转换为 num_classes的one_hot形式
        loss = self.toVariable(target_expanded) * inputs
